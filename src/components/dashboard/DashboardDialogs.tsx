    ...
    <>
      ...
      <Dialog open={ouvert} onOpenChange={setOuvert}>
        <DialogContent className="max-w-[400px] gap-0 overflow-hidden rounded-card border border-line bg-white p-0 shadow-lg">
          <div className="flex items-start justify-between border-b border-surface-soft px-4 py-3">
            <div>
              <DialogTitle className="text-sm font-medium text-ink">{bienNom}</DialogTitle>
            ...
            {missions.map((m) => (
              <li key={m.id}>
                <button
                  ...
                  className="flex w-full items-start gap-3 border-b border-surface-soft px-4 py-3 text-left last:border-b-0 hover:bg-surface"
                >
                  <span className="text-base leading-6">{m.emoji}</span>
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center justify-between gap-2">
                      <span className="text-xs font-medium text-ink">{m.titre}</span>
                      <span
                        className={cn(
          ...
          <div className="border-t border-surface-soft px-4 py-3">
            <button
              ...
              className="h-[30px] rounded border border-line-strong px-4 text-xs font-medium text-ink-body"
            >
              Fermer
            </button>
          </div>
...
export function MissionInfoDialog({
  ...
  return (
    ...
    >
      <DialogContent
        className={cn(
          "gap-0 overflow-hidden rounded-card border border-line bg-white p-0 shadow-lg",
      ...
      >
        {details ? (
          ...
          <>
            <div className="flex items-center gap-2 border-b border-surface-soft px-5 py-4">
              <ClipboardList className="size-4 text-ink-body" />
            ...
            <div className="px-5 py-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-base text-ink">{mission.titre}</p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <div className="rounded-card border border-surface-soft p-2.5">
                  <p className="text-xs text-ink-muted">Bien</p>
                  <p className="mt-1 text-xs text-ink">{bienNom}</p>
                </div>
                <div className="rounded-card border border-surface-soft p-2.5">
                  <p className="text-xs text-ink-muted">Date & heure</p>
                ...
                <div className="col-span-2 rounded-card border border-surface-soft p-2.5">
                  <p className="text-xs text-ink-muted">Assigné à</p>
                  <p className="mt-1 text-xs text-ink">{mission.assigne}</p>
                </div>
              </div>
              <div className="mt-2 rounded-card border border-surface-soft p-2.5">
                <p className="text-xs text-ink-muted">Description</p>
                <p className="mt-1 text-xs leading-5 text-ink-body">{mission.description}</p>
              </div>
            </div>
    ...
    <div>
      <div className="flex items-start justify-between px-4 pt-4">
        <div className="flex items-center gap-3">
          <span className="flex size-12 items-center justify-center rounded-card bg-line text-lg text-ink-body">
            J
          </span>
          <div>
            <DialogTitle className="text-lg font-medium text-ink">Clean Up</DialogTitle>
            <p className="text-sm text-ink-muted">ID #12345</p>
          </div>
        </div>
        <button type="button" onClick={onFermer} aria-label="Fermer" className="text-ink-muted">
          <X className="size-4" />
        </button>
      </div>
      <DialogDescription className="px-4 pt-2 text-sm text-ink-subtle">
        Détails de la réservation et informations du locataire
      </DialogDescription>
      <div className="mx-auto mt-3 flex h-[29px] w-[305px] items-center justify-center rounded-full bg-surface-soft text-xs text-ink-body">
        Validé
      </div>
      <button
        type="button"
        className="mt-4 flex w-full items-center justify-between border-t border-surface-soft px-4 py-4 text-left"
        onClick={() => setOuvert((o) => ({ ...o, desc: !o.desc }))}
      >
        <span className="text-base text-ink">Descriptions</span>
        <span className="text-ink-muted">▼</span>
      </button>
      {ouvert.desc && (
        <div className="space-y-3 px-4 pb-4 text-sm">
          <p className="text-ink">★ Prestations et services</p>
          <div className="grid grid-cols-3 gap-3 text-xs">
            <div>
              <p className="text-ink-muted">Type appartement</p>
              <p className="text-ink">2 pièces</p>
            </div>
            <div>
              <p className="text-ink-muted">Montant</p>
              <p className="text-ink">30 euros</p>
            </div>
            <div>
              <p className="text-ink-muted">Lavage du linge</p>
              <p className="text-ink">20 euros</p>
            </div>
            <div>
              <p className="text-ink-muted">Installation du linge</p>
              <p className="text-ink">Inclus</p>
            </div>
            <div>
              <p className="text-ink-muted">Consommables</p>
              <p className="text-ink">Inclus</p>
            </div>
            <div>
              <p className="text-ink-muted">Autres</p>
              <p className="text-ink">Lit bébé</p>
              <p className="text-ink">Installation</p>
            </div>
            <div>
              <p className="text-ink-muted">Montant</p>
              <p className="text-ink">10 euros</p>
              <p className="text-ink">10 euros</p>
            </div>
            <div>
              <p className="text-ink-muted">Comissions</p>
              <p className="text-ink">10%</p>
            </div>
            <div>
              <p className="text-ink-muted">Montant</p>
              <p className="text-ink">200 euros</p>
            </div>
          </div>
        </div>
      )}
      <button
        type="button"
        className="flex w-full items-center justify-between border-t border-surface-soft px-4 py-4 text-left"
        onClick={() => setOuvert((o) => ({ ...o, periode: !o.periode }))}
      >
        <span className="text-base text-ink">Période de la prestation</span>
        <span className="text-ink-muted">▼</span>
      </button>
      {ouvert.periode && (
        <div className="grid grid-cols-3 gap-3 px-4 pb-6 text-xs">
          <div>
            <p className="text-ink-muted">Prestatation réalisée par:</p>
            <p className="mt-1 text-ink">Clean Up</p>
          </div>
          <div>
            <p className="text-ink-muted">Date de début:</p>
            <p className="mt-1 text-ink">05/03/2026</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-ink-muted">Récurrence</p>
            <p className="mt-1 text-ink">Hebdomadaire</p>
          </div>
          <div className="col-span-2">
            <p className="text-ink-muted">Mission</p>
            <p className="mt-1 text-ink">{mission.titre}</p>
          </div>
        </div>
      )}
    </div>
...
export function CreateEventDialog({
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[90vh] max-w-[720px] overflow-y-auto rounded-card border border-line bg-white p-6">
        <DialogTitle className="text-sm font-medium text-ink">Créer un événement</DialogTitle>
        <div className="space-y-4">
          <label className="block">
            <span className="text-sm text-ink">Nom de l'événement *</span>
            <input
              value={nom}
              onChange={(e) => setNom(e.target.value)}
              placeholder="Ex: Festival de Jazz, Marathon, Salon professionnel..."
              className="mt-2 h-[42px] w-full rounded-card border border-line px-4 text-sm text-ink outline-none placeholder:text-ink-muted"
            />
          </label>
          <label className="block">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                className="mt-2 h-[42px] w-full rounded-card border border-line px-4 text-sm text-ink outline-none"
              />
            </label>
            <label className="block">
              <span className="flex items-center gap-2 text-sm text-ink">
          ...
          <label className="block">
            <span className="text-sm text-ink">Type d'impact</span>
            <select
              value={impact}
              onChange={(e) => setImpact(e.target.value as ImpactEvenement)}
              className="mt-2 h-[41px] w-full rounded-card border border-line bg-white px-4 text-sm text-ink outline-none"
            >
          ...
          <label className="block">
            <span className="text-sm text-ink">Description / Notes</span>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="Décrivez l'impact potentiel de cet événement sur vos propriétés..."
              className="mt-2 h-[114px] w-full resize-none rounded-card border border-line px-4 py-2 text-sm text-ink outline-none placeholder:text-ink-muted"
            />
          </label>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
          ...
          <div className="flex w-full flex-wrap gap-2 sm:w-auto">
            <button
              type="button"
              onClick={onFermer}
              className="h-9 rounded-card border border-line-strong px-4 text-sm text-ink-body"
            >
              Annuler
            </button>
            <button
              ...
              className="inline-flex h-9 items-center gap-2 rounded-card bg-ink px-4 text-sm font-medium text-white disabled:opacity-50"
            >
...
export function CreateRegleDialog({
  ...
  return (
    <Dialog open={ouvert} onOpenChange={(o) => !o && onFermer()}>
      <DialogContent className="max-h-[90vh] max-w-[520px] overflow-y-auto rounded-card border border-line bg-white p-0">
        <div className="flex items-center justify-between border-b border-surface-soft px-5 py-4">
          <DialogTitle className="flex items-center gap-2 text-sm font-medium text-ink">
        ...
        <div className="space-y-4 px-5 py-4">
          <label className="block">
            ...
            <input
              ...
              className="mt-2 h-[34px] w-full rounded-card border border-line px-3 text-xs text-ink outline-none placeholder:text-ink-muted"
            />
          </label>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="flex items-center gap-1 text-xs text-ink-body">
              ...
              <input
                ...
                value={debut}
                onChange={(e) => setDebut(e.target.value)}
                className="mt-2 h-[34px] w-full rounded-card border border-line px-3 text-xs outline-none"
              />
            </label>
            <label className="block">
              <span className="flex items-center gap-1 text-xs text-ink-body">
          ...
          <label className="block">
            <span className="text-xs text-ink-body">Bien(s) concerné(s)</span>
            <select
              value={bienId}
          ...
          <div>
            <p className="text-xs text-ink-body">Type de modification tarifaire</p>
          ...
          <label className="block">
            <span className="text-xs text-ink-body">Valeur (%)</span>
          ...
          <label className="block">
            <span className="text-xs text-ink-body">Note / commentaire (optionnel)</span>
            <textarea
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder="Ex : Marché de Noël, festival local…"
              className="mt-2 h-[50px] w-full resize-none rounded-card border border-line px-3 py-2 text-xs outline-none placeholder:text-ink-muted"
            />
          </label>
          <div className="rounded-card border border-line p-3">
            <p className="text-xs text-ink-body">Ajouter à un ensemble de règles</p>
            <div className="mt-2 space-y-1">
              {ensembles.map((e) => (
                <label
                  key={e.id}
                  className="flex items-center gap-2 rounded-[8px] px-2 py-2 text-xs hover:bg-surface"
                >
                  <input
                    type="radio"
                    name="ensemble"
    ...
    <Dialog open={ouvert} onOpenChange={(o) => !o && onFermer()}>
      <DialogContent className="max-h-[90vh] max-w-[400px] overflow-y-auto rounded-card border border-line bg-white p-0">
        ...
        <div className="space-y-3 px-4 pb-6">
          {parEnsemble.map((e) => (
            <div key={e.id} className="overflow-hidden rounded-card border border-line">
              <div className="flex items-start gap-2 px-3 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-2 text-xs text-ink">
                    {e.nom}
                    {!e.actif && (
                      <span className="rounded border border-line px-1.5 text-[10px] text-ink-muted">
                        Inactif
                      </span>
                    )}
                  </p>
                  <p className="text-[10px] text-ink-muted">{e.description}</p>
                </div>
              ...
              {(
                <div className="border-t border-surface-soft">
                  {e.items.map((r) => (
                    <div
                      key={r.id}
                      className="flex items-start gap-2 border-b border-surface-soft px-3 py-2.5 last:border-b-0"
                    >
                      <span className="mt-1 size-2.5 rounded-full bg-line" />
                      <div className="min-w-0 flex-1">
                        ...
                        {r.note && <p className="text-[10px] text-ink-muted">{r.note}</p>}
                      </div>
                      <span
                        className={cn(
                  ...
                  <button
                    ...
                    className="flex h-8 w-full items-center justify-center gap-1 text-xs text-ink-body"
                  >
